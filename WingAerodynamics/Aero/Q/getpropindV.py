...
def load_config(matfile, viscous=False,skinfriction=True, iter_max=1):
    ...
    class Options:
        settings = {'dJ': 0.8,                      # 0.5
                         'N_prop_map': 3,           # 3
                         'slipstream_length': 15.5,    # 2 # at least (c4 + x)*2
                         'dlbda': 0.1,              # 0.1
                         'dlb': 0.005,              # 0.005
                         'p_max': 10,               # 10
                         'lbda_max': 5,             # 5
                         'N_time_step': 30,         # 30
                         'N_slipstream_x': 70,      # 12 # 100 !!!!!!!!!!!!!!!!
...
def main(matfile='ATR72.mat', alpha=0, viscous=False, skinf=True, iter_max=1, props=[], condition='pullup'):
    fc, ib, wt, wing, options = load_config(matfile, viscous, skinf, iter_max)
    wing.alpha = alpha

    # if condition=='pullup':
    #     fc.M = fc.M + 0.04
    #     fc.Vinf = fc.M*fc.a
    #     fc.alt = fc.alt + 3000 * 0.3048 # 3000ft to m
    #     print('2.G pullup load case')
    ib.z = 0
    if 'ib' in props and 'wt' in props:
        propellers = [ib, wt]  # propellers to be taken into account
        wing.m = np.array([9, 29, 9, 30, 16]) #np.array([12, 33, 12, 22, 16])
        wing.n = np.array([24, 24, 24, 24, 24])#np.array([36, 36, 36, 36, 36])
        wing.opt = ['nsin', 'uni', 'sin', 'nsin', 'uni']
    elif 'wt' in props:
        propellers = [wt]
        wing.n = np.array([24,24])
        wing.m = np.array([30,10])
        wing.opt = ['cos', 'uni']
    elif 'ib' in props:
        propellers = [ib]
        wing.n = np.array([24,24,24,24])
        wing.m = np.array([10,11,10,15])
        wing.opt = ['nsin', 'uni', 'sin', 'nsin']
    elif props ==[]:
        wing.n = np.array([24])
        wing.m = np.array([30])
        wing.opt = ['cos']
    ...
    T = []
    for prop in wingsys.propellers:
        print(prop.bem.res_sum)
        t = prop.bem.res_sum['T']
        T.append(t)
    return wingsys


if __name__ == '__main__':
    res = main('Temp/Temp_design.mat', 0, viscous=False, skinf=False, props=['ib', 'wt'], iter_max=1)
    # 'Design/Modified/original/TP_tip_high_02_kink0_ATRairfoil__0.200_0.8735_0.25_0.030_.mat'
    df = pd.DataFrame(columns= ['y','V1x_ib', 'V2x_ib', 'V1z_ib', 'V2z_ib', 'V1x_wt', 'V2x_wt','V1z_wt', 'V2z_wt'])
    df.V1x_ib = res.V1x_lst_ib
    df.V2x_ib = res.V2x_lst_ib
    df.V1z_ib = res.V1z_lst_ib
    df.V2z_ib = res.V2z_lst_ib
    df.V1x_wt = res.V1x_lst_wt
    df.V2x_wt = res.V2x_lst_wt
    df.V1z_wt = res.V1z_lst_wt
    df.V2z_wt = res.V2z_lst_wt
    df.y = res.vlm.Vpert.y.values
    df.to_csv('propindvel.dat')
