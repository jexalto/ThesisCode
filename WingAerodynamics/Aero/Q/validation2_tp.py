...
class WTMP():
    ...
    y =  0.730 * 2 * 0.952 / 2 # spanwise position prop y/b/2
...
# --------------- WTM Propeller ----------------
class InboardPropeller:  # disregarded
...
class Wing:
    ...
    n = [24, 24]  # chordwise panels per section
...
aoa = np.linspace(-8,10,19)

def val_wt(ps, aoa, name =None):
    ind = 0
    for p in ps:
        # for p in [0, 2, 4]:
        if name is None:
        ...
        if p == 0:
            ...
            for alpha in aoa:
                ...
                wt.Vinf = fig24[fig24['polar'] == p + 1]['Vinf'].mean()
                wt.omega = fig24[fig24['polar'] == p + 1]['n'].mean() * 2 * np.pi
                ...
                wt_prop = wingsys.propellers[0].urot.prop_us
                bem_wt = wingsys.propellers[0].bem
                vlm = wingsys.vlm

                L_t = wt_prop['integral_T'] * np.sin(np.deg2rad(alpha)) + wt_prop['integral_delta_Fz'] * np.cos(
                    np.deg2rad(alpha))
                D_t = -wt_prop['integral_T'] * np.cos(np.deg2rad(alpha)) + wt_prop['integral_delta_Fz'] * np.sin(
                    np.deg2rad(alpha))
                ...
                bem_dct[alpha] = copy.deepcopy(bem_wt)
