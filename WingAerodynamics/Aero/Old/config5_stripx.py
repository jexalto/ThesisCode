...
if __name__ == '__main__':
    ...
    import time
    import pickle
    n = np.array([8, 8,8,8, 8])
    for i in np.array([ 0.5, 1, 1.25, 1.5, 2,2.5]): #0.5, 0.75, 1.25,
        fc = FlightConditions()
        ib = InboardPropeller()
        wt = WTMP()
        wing = Wing()
        options = Options()
        wing.n = np.array([70, 70, 70, 70, 70])
        ...
        # if wing.n[1]%2 ==0:
        #     wing.n[1] = wing.n[1] +1
        wing.n = wing.n.astype(int).tolist()
        ...
        print(vlm.res)
        print(vlm.time)
        with open('vlm_convstudy_stripxI2', 'wb') as conv_file:
            pickle.dump(vlm_dct, conv_file)

        print('Next')
        del fc, ib, wt, wing, wingsys, propellers
    print('Ready')
