...
from jet_correction_tip import jet_correction_tip
...
class PyVLM(object):
    ...
    def __init__(self):
        ...
        self.ib_loc = None
        self.wt_R = None        # wingtip propeller radius
        self.ib_R = None
    ...
    def add_wing(self, lead_edge_coord, chord_lengths, n, m, opt=None):
        ...
        for k in range(Nle - 1):
            ...
            if k==0:
                ...
                Panels_ = mesh.panels(count=sum(m[:k]))     # CHANGED
        ...
        for k in range(Nle - 1):
            ...
            if k==0:
                ...
                Panels_ = mesh.panels(ismirror=True,count=sum(m[:k])) # CHANGED
...
#        self.Vpert['dalpha'] = 0
        ...
        for y in Strip.index:
            ...
            if np.isnan(cl_visc):
                print('nan')
                #cl_visc = 0
            cl_inv = self.airfoil.cl_thin_airfoil(alpha_eff, M)
            
            #translate difference in CL to twist
            dcl_da = (2*np.pi*np.pi/180) * 1/ np.sqrt(1-M**2)
    ...
    def vlm(self):
        ...
        for i,panel in enumerate(Panels):
            ...
            panel.Vinf_n = Vinf * (panel.slope-dalpha-alpha) + panel.slope*panel.Vx - panel.Vz  # small angle approximation
            Vinf_n[i] = panel.Vinf_n
       #     mach[i] = panel.V_eff/self.a
        ...
        if (type(self.G) == int):
            ...
            for i in range(1, y.size-1):
                ...
                ib_loc = self.ib_loc
                ib_R = self.ib_R
                # check for significant velocity difference
                if round(mu_r, 2) != 1:
                    # check if wt or ib slipstream
                    if y[i] > max(y) - self.wt_R *1.1:
                        Gi = jet_correction_tip(y, mu_r, panel_chord, r,
                                            self.jet_corr_settings['dlbda'],
                                            self.jet_corr_settings['dlb'],
                                            self.jet_corr_settings['p_max'],
                                            self.jet_corr_settings['lbda_max'])
                        G_wt += Gi
                    elif type(ib_loc)==np.ndarray:    # only if ib exists

                        if ib_loc[1]-ib_R <y[i] < ib_loc[1] and mu_r<1:     # assume rings of constant jet velocity, jet correction only required on one side
                            Gi = jet_correction_ib(y, mu_r, panel_chord, loc, ib_loc[1],
                                                self.jet_corr_settings['dlbda'],
                                                self.jet_corr_settings['dlb'],
                                                self.jet_corr_settings['p_max'],
                                                self.jet_corr_settings['lbda_max'])
        ...
        gamma = np.linalg.solve(self.AIC-self.G, Vinf_n)
        mach = np.ones(N) * self.Vinf/self.a
        gamma = gamma*1/(np.sqrt(1-mach**2))        # compressibility correction
        self.gamma = gamma
        ...
        for i,panel in enumerate(Panels):
            ...
            # moment about wing quarter chord
            panel.m = Fz * (panel.C4[0] - (panel.A[0]+panel.B[0])/2)
    ...
    def strip_properties(self, skin_fric=True):
        ...
        cols = ['l', 'cl', 'd', 'cd', 'cm', 'di', 'di_', 'cdi', 'cdi_', 'dp', 'cdp', 'm', 'area', 'span',
                'chord', 'gamma', 'V_eff', 'alpha_eff', 'V_ind', 'Vx', 'Vz',
                'Re', 'C4', 'LE', 'x_coord', 'turb']
        #summation over panels
        cols_sum = ['l', 'di', 'di_', 'area', 'gamma', 'm']
        ...
        data['cm'] = data['m'] / (data['area'] * q_inf*data['chord'])
