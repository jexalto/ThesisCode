...
from EOAS.EOAS_group_aero import EOAS
...
class master(om.Group):
    ...
    def setup(self):
        ...
        self.connect('helix.rotorcomp_0_radii',                         'EOAS.wing.mesh.jet_radius')
        ...
        self.connect('linear_radius.chord_list',                        'EOAS.wing.chord_cp')
        self.connect('parameters.jet_loc',                              'linear_radius.jet_loc')
        self.connect('linear_radius.jet_loc_list',                      'EOAS.wing.mesh.jet_loc')
        self.connect('parameters.span',                                 'EOAS.wing.span')
        ...
        self.connect('EOAS.AS_point_0.CL',                    'constraints.L')
        self.connect('parameters.rho',                                  'constraints.rho')
        self.connect('parameters.vinf',                                 'constraints.V')
        self.connect('EOAS.AS_point_0.wing.S_ref',                      'constraints.surface')
    ...
    def configure(self):
        ...
        if True:
            for iParametric in range(0, np.size(parametric)):
                ...
                self.add_design_var(name + "twist",lower=10, upper=70, scaler=1/10)
        ...
        self.add_design_var('parameters.twist',                     lower=-10, upper=10, scaler=1)

        self.add_objective("obj_function.objective",                scaler=1000)
...
prob.set_val(name + "twist",val=twist)#np.array([70.        , 68.20803018, 64.56277741, 55.70852641, 54.95242797,
    #    49.19341101, 47.81055488, 44.30355166, 41.64782961, 39.85986054,
    #    39.75825531, 35.09467322, 36.64626379, 32.25124359, 32.08916242,
    #    31.27566964, 29.2067543 , 28.29093411, 25.47465242, 23.7263215 ]))#twist)
...
prob.driver.opt_settings = {
    ...
    "Print file": "00_results/snopt_output/opt_SNOPT_print_aero.txt",
    "Summary file": "00_results/snopt_output/opt_SNOPT_summary_aero.txt",
}
...
prob.run_driver()
# prob.check_partials(compact_print=True, show_only_incorrect=False, includes=['*EOAS.wing.mesh*', '*rethorst*'], form='central', step=1e-8) # excludes=['*parameters*, *helix*, *EOAS*, *rethorst*']
