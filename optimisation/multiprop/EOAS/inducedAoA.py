...
class propinflow(om.ExplicitComponent):
    def initialize(self):
        self.options.declare('nx', default=2, 
                                desc='Number of chordwise discretisation points')
        self.options.declare('ny', default=101, 
                                desc='Number of spanwise discretisation points')
        self.options.declare('nr_props', default=1, 
                                desc='Number of propellers')
        self.options.declare('propdist_chord', default=0.2, 
                                desc='Chordwise distance of prop versus leading edge')
    ...
    def setup(self):
        ...
        self.add_input('vinf', val=1.0)

        self.add_output('propinflow', val=[0, 0.81051701, -40])
    ...
    def compute(self, inputs, outputs):
        ...
        iCirculation    = np.argmin(abs(y-jet_loc[0]))
        ...
        # print('INDUCED VELOCITY: ', v_ind)

        outputs['propinflow'] = induced_aoa
