...
class master(om.Group):
    ...
    def setup(self):
        ...
        nx = 2
        ny=101
        self.add_subsystem('rethorst', subsys=Rethorst(span_max=span_max, vel_distr_shape=N_elem_span, panels_span_VLM=ny-1, panels_chord_VLM=1))
        ...
        self.connect('helix.simparamdef_v_inf',                         'prop_weight.power')
    ...
    def configure(self):
        ...
        if True:
            for iParametric in range(0, np.size(parametric)):
                ...
                # self.add_design_var(name + "twist",lower=0, upper=75)
        ...
        self.add_design_var('parameters.jet_loc', lower=[-0.748/2, 0.1], upper=[-0.1, 0.748/2])
        ...
        # self.add_constraint('EOAS.AS_point_0.wing_perf.L', equals=16)
