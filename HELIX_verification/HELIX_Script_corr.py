...
import HELIX_component as helix_comp
...
def main():
    ...
    V_inf = np.array([5.0, 10.0, 15.0])
    V_inf_tag = ["V_05", "V_10", "V_15"]

    AOA = np.array([90.0, 80.0, 70.0, 60.0, 50.0, 40.0, 30.0, 20.0, 10.0, 0.0])
    AOA_tag = ["AOA_90", "AOA_80", "AOA_70", "AOA_60", "AOA_50", "AOA_40", "AOA_30", "AOA_20", "AOA_10", "AOA_00"]
    ...
    # =========================================================================
    with open("./Data-MR8x45.json") as f:
        wt_data = json.load(f)
    ...
    for iCase in range(0, np.size(V_inf)):
        for iAOA in range(0, np.size(AOA)):
            ...
            iRPM = 0
            while True:
                if wt_data[V_inf_tag[iCase]][AOA_tag[iAOA]]["thrust"][iRPM] > tol:
                    break
                elif iRPM == N_RPM - 1:
                    raise IndexError("RPM index is out of bounds, check to ensure thrust data is positive")
                else:
                    iRPM += 1
    ...
    if correction:
        plt.savefig('/Users/joaquinexalto/Documents/TU_Delft/code/HELIX_verification/data/data_WITHcorrection.png')

        # --- Write thrust data to txt file ---
        file = open("/Users/joaquinexalto/Documents/TU_Delft/code/HELIX_verification/data/data_WITHcorrection.txt", "w")
        file.write("%s = %s\n" %('V=15', thrust[2, -1, :]))

        file.close()
    
    else:
        plt.savefig('/Users/joaquinexalto/Documents/TU_Delft/code/HELIX_verification/data/data_NOcorrection.png')

        # --- Write thrust data to txt file ---
        file = open("/Users/joaquinexalto/Documents/TU_Delft/code/HELIX_verification/data/data_NOcorrection.txt", "w")
        file.write("%s = %s\n" %('V=15', thrust[2, -1, :]))
