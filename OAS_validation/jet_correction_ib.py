...
def jet_correction_ib(y, mu, crd, loc, ib_loc, dlbda=0.1, dlb=0.005, p_max=10, lbda_max=5):
    ...
    Args:
        ...
        loc (float): location --> actually radius
    ...
    #y = np.sort(-y)     # invert y and sort from -b/2 to 0
    y_ = (y[1:]+y[:-1])/2   # centre of each panel
    
    #calculations are made in a system where the jet is on y=0
    y2 = y_- ib_loc+s0          # locations of control point
    y2_ = y - ib_loc+s0          # locations of hb vortex points
    ...
    #horseshoe vortex midpoint y coordinates
    beta = y2[n:]/r0_
